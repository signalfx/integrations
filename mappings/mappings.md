...
| `disk_merged.read` (metric)                               | `system.disk.merged` (metric) with dimension name `direction` equal to `read`            |
| `disk_merged.write` (metric)                              | `system.disk.merged` (metric) with dimension name `direction` equal to `write`           |
| `disk_octets.read` (metric)                               | `system.disk.io` (metric) with dimension name `direction` equal to `read`                |
| `disk_octets.write` (metric)                              | `system.disk.io` (metric) with dimension name `direction` equal to `write`               |
| `disk_ops.read` (metric)                                  | `system.disk.operations` (metric) with dimension name `direction` equal to `read`        |
| `disk_ops.write` (metric)                                 | `system.disk.operations` (metric) with dimension name `direction` equal to `write`       |
| `disk_time.read` (metric)                                 | `system.disk.time` (metric) with dimension name `direction` equal to `read`              |
| `disk_time.write` (metric)                                | `system.disk.time` (metric) with dimension name `direction` equal to `write`             |
| `host` (dimension)                                        | `host.name` (dimension)                                                                  |
| `if_dropped.rx` (metric)                                  | `system.network.dropped` (metric) with dimension name `direction` equal to `receive`     |
| `if_dropped.tx` (metric)                                  | `system.network.dropped` (metric) with dimension name `direction` equal to `transmit`    |
| `if_errors.rx` (metric)                                   | `system.network.errors` (metric) with dimension name `direction` equal to `receive`      |
| `if_errors.tx` (metric)                                   | `system.network.errors` (metric) with dimension name `direction` equal to `transmit`     |
| `if_octets.rx` (metric)                                   | `system.network.io` (metric) with dimension name `direction` equal to `receive`          |
| `if_octets.tx` (metric)                                   | `system.network.io` (metric) with dimension name `direction` equal to `transmit`         |
| `if_packets.rx` (metric)                                  | `system.network.packets` (metric) with dimension name `direction` equal to `receive`     |
| `if_packets.tx` (metric)                                  | `system.network.packets` (metric) with dimension name `direction` equal to `transmit`    |
...
| `kubernetes.container_memory_limit` (metric)              | `k8s.container.memory.limit` (metric)                                                    |
...
| `kubernetes.cronjob.active` (metric)                      | `k8s.cronjob.active_jobs` (metric)                                                       |
| `kubernetes.daemon_set.current_scheduled` (metric)        | `k8s.daemonset.current_scheduled_nodes` (metric)                                         |
| `kubernetes.daemon_set.desired_scheduled` (metric)        | `k8s.daemonset.desired_scheduled_nodes` (metric)                                         |
| `kubernetes.daemon_set.misscheduled` (metric)             | `k8s.daemonset.misscheduled_nodes` (metric)                                              |
| `kubernetes.daemon_set.ready` (metric)                    | `k8s.daemonset.ready_nodes` (metric)                                                     |
| `kubernetes.deployment.available` (metric)                | `k8s.deployment.available` (metric)                                                      |
| `kubernetes.deployment.desired` (metric)                  | `k8s.deployment.desired` (metric)                                                        |
| `kubernetes.hpa.spec.max_replicas` (metric)               | `k8s.hpa.max_replicas` (metric)                                                          |
| `kubernetes.hpa.spec.min_replicas` (metric)               | `k8s.hpa.min_replicas` (metric)                                                          |
| `kubernetes.hpa.status.current_replicas` (metric)         | `k8s.hpa.current_replicas` (metric)                                                      |
| `kubernetes.hpa.status.desired_replicas` (metric)         | `k8s.hpa.desired_replicas` (metric)                                                      |
| `kubernetes.job.active` (metric)                          | `k8s.job.active_pods` (metric)                                                           |
| `kubernetes.job.completions` (metric)                     | `k8s.job.desired_successful_pods` (metric)                                               |
| `kubernetes.job.failed` (metric)                          | `k8s.job.failed_pods` (metric)                                                           |
| `kubernetes.job.parallelism` (metric)                     | `k8s.job.max_parallel_pods` (metric)                                                     |
| `kubernetes.job.succeeded` (metric)                       | `k8s.job.successful_pods` (metric)                                                       |
...
| `kubernetes.replica_set.available` (metric)               | `k8s.replicaset.available` (metric)                                                      |
| `kubernetes.replicaset.desired` (metric)                  | `k8s.replicaset.desired` (metric)                                                        |
| `kubernetes.replication_controller.available` (metric)    | `k8s.replication_controller.available` (metric)                                          |
| `kubernetes.replication_controller.desired` (metric)      | `k8s.replication_controller.desired` (metric)                                            |
| `kubernetes.resource_quota_hard` (metric)                 | `k8s.resource_quota.hard_limit` (metric)                                                 |
| `kubernetes.resource_quota_used` (metric)                 | `k8s.resource_quota.used` (metric)                                                       |
| `kubernetes.stateful_set.current` (metric)                | `k8s.statefulset.current_pods` (metric)                                                  |
| `kubernetes.stateful_set.desired` (metric)                | `k8s.statefulset.desired_pods` (metric)                                                  |
| `kubernetes.stateful_set.ready` (metric)                  | `k8s.statefulset.ready_pods` (metric)                                                    |
| `kubernetes.stateful_set.updated` (metric)                | `k8s.statefulset.updated_pods` (metric)                                                  |
